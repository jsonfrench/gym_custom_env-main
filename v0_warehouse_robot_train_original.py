...
def run_q(episodes, is_training=True, render=False):
    ...
    x_pos_space = np.linspace(env.observation_space.low[0], env.observation_space.high[0], 20)    # Between 0 and 5
    y_pos_space = np.linspace(env.observation_space.low[0], env.observation_space.high[0], 20)    # Between 0 and 5

    if(is_training):
        # If training, initialize the Q Table, a 5D vector: [robot_row_pos, robot_row_col, target_row_pos, target_col_pos, actions]
        q = np.zeros((len(x_pos_space), len(y_pos_space), env.unwrapped.grid_rows, env.unwrapped.grid_cols, env.action_space.n))
    else:
    ...
    for i in range(episodes):
        ...
        state = env.reset()[0]
        state_x = np.digitize(state[0], x_pos_space)
        state_y = np.digitize(state[1], y_pos_space)
        ...
        while(not terminated):
            ...
            new_state_x = np.digitize(state[0], x_pos_space)
            new_state_y = np.digitize(state[1], y_pos_space)
            ...
            new_state = [new_state_x, new_state_y, int(new_state[2]), int(new_state[3])]
            
            #print("HELP MEEEEEEEEEEEEEEEEEEE!")
            #print(state)
            #print("new state:", new_state)

            q_new_state_idx = tuple(new_state)
            
            #print(q_new_state_idx)
